function [OUT] = Bayes_ZIL_T_MS_1l_TEST(Xt,Zt, nameArgs)
%% Bayes_ZIL_T_MS_1l_TEST Estimate the Bayesian Zero-Inflated Logit Tensor model with Markov Switching coefficients (ZIL-T-MS) with common covariates and K=1 layers
%
%   [OUT] = Bayes_ZIL_T_MS_1l_TEST(XT,ZT,NAMEARGS)
...
%   The observations consists in binary arrays XT of size (I,J,T) and covariates ZT of size (Q,T).
...
%                      * gamma3hat  (Q,R,L) CP marginal vector of mode 3, for each r=1..R and state l=1..L
...
arguments
   ...
   nameArgs.R        (1,1) double = 6      % coefficient tensor CP rank
...
if size(Xt,ndims(Xt)) ~= size(Zt,ndims(Zt))
   error('Xt must be (I,J,T) and Zt must be (Q,T).')
end
...
psihat = ones(R,1);     %dhat = zeros(I,J,T);
dhat = zeros(I*J,T);
u    = zeros(I*J,T);
uminusg = zeros(I*J,T);
...
OUT.gamma3hat = zeros(Q,R,L,NumIter);
...
for iter=1:TotIter   
   ...
   [~,~,~,smoothp,sthat,~] = ffbsX(Xt,Zt,rhohat,Ghat,xihat,iphat,0);
...
%       Glz   = double( ttv(Ghat(:,:,:,:,sthat(t)), Zt(:,t), 4) );   % select tensor for state t --> IxJQ
      Glz    = ttv_mod(Ghat(:,:,:,sthat(t)), Zt(:,t), 3);   % select tensor for state t --> IxJQ
...
%             vecGlrZt = double( ttv(GlrL1, Zt(:,t), 3) );
            vecGlrZt = ttv_mod(GlrL1, Zt(:,t), 3);
            uminusg(:,t) = u(:,t) - vecGlrZt(:);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            gam3Zt(t) = gamma3hat(:,r,ll)' * Zt(:,t);
            tmp = Omegabarbar(:,1+I*J*(t-1):I*J*t) * kron2L1;
            sumSigma1L1   = sumSigma1L1   + (kron2L1' * tmp * gam3Zt(t)^2);
            sumMuSigma1L1 = sumMuSigma1L1 + (gam3Zt(t) * uminusg(:,t)' * tmp)';
%          end
      end
...
%       for t=1:T
%          if (sthat(t) == ll)
            tmp = Omegabarbar(:, 1+I*J*(t-1):I*J*t) * kron1L1;
            sumSigma2L1   = sumSigma2L1   + (kron1L1' * tmp * gam3Zt(t)^2);
            sumMuSigma2L1 = sumMuSigma2L1 + (gam3Zt(t) * uminusg(:,t)' * tmp)';
%          end
      end
...
%       gamma2hat(:,r,ll) = mvnrnd(mu2postL1, Sigma2postL1);
   ...
   %          if (sthat(t) == ll)
            ...
            tmp = Omegabarbar(:, 1+I*J*(t-1):I*J*t) * kron1L1;
            sumSigma3L1   = sumSigma3L1   + Ztgam2 * kron1L1' * tmp * Ztgam2';
            sumMuSigma3L1 = sumMuSigma3L1 + (uminusg(:,t)' * tmp * Ztgam2')';
%          end
      end

function [lkl,filtp,filts,smoothp,smooths,clkl] = ffbsX_tensorData(Xt,Zt,rho,G,P,ip,scale_clkl)
%% ffbsX_tensorData Forward-Filtering Backward-Sampling for ZIL-T-MS model with tensor observations and common covariates
%
...
%
% INPUTS
%  Xt   IxJxKxT    binary array of size IxJxK of observations for each time t=1..T
%  Zt   QxT        vector of size Qx1 of covariates for each time t=1..T
%  G    IxJxKxQxL  coefficient tensor IxJxQ for each state l=1..L
%  P    LxL        transition matrix for the hidden Markov chain
%  ipm  Lx1        initial probabilities
%  scale_clkl = 1  rescale clkl until difference proba between states < 2.5
%
% OUTPUTS
%  lkl      Tx1  unconditional log-likelihood   log(p(Xt|Xt-1,..,X1))
...
%       logpX = (logpX1 .* (Xt(:,:,:,t)==1)) + (logpX0 .* (Xt(:,:,:,t)==0));
%       clkl(t,l) = sum(logpX,'all');     % sum since likelihood factorises for each entry
   end
end
...
% function [lkl,filtp,filts,smoothp,smooths,clkl] = ffbsX_tensorData(Xt,Zt,rho,G,P,ip,scale_clkl)
% %% Forward Filtering Backward Sampling - for MS binary tensor model with 1 equation
% % Samples trajectory (s1,...,sT):
% % 1) computes conditional log-likelihood (specific for each model)
% % 2) runs FFBS algorithm: FF forward filtering; BS backward smoothing and sampling
% %
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % Regimes cycle: computes log-conditional likelihood, then 'Hamilton' for FFBS   
%    [I,J,K,T] = size(Xt);      % T = length time series (observations)
%    L = size(P,1);           % L = #regimes
%    clkl_X = zeros(T,L);
%    
%    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%    %             ( p(XT,yT|sT=1) ... p(XT,yT|sT=L) )
%    for t=1:T
%       logden = zeros(I,J,K,L);
%       logpX1 = zeros(I,J,K,L);
%       logpX0 = zeros(I,J,K,L);
%       logpX  = zeros(I,J,K,L);
%       for l=1:L
%          %%% conditional likelihood (Xt|st,-) -- discrete 0-1 %%%
%          Gz = double( ttv(G(:,:,:,:,l), Zt(:,t), 4) );
...
%             % P(x_ij = 1|-) = (1-rho) * exp(Gz)/(1+exp(Gz))
%             % P(x_ij = 0|-) = rho + (1-rho) * 1/(1+exp(Gz)) = (1+rho*exp(Gz)) / (1+exp(Gz))
% %   logden(:,:,l) = log(1+exp(Gzsmall)).*psmall + (log(1+1./exp(Gzbig))+Gzbig).*pbig;    % distance ~10^-7 with:  Gzbig
% %   logpX1(:,:,l) = log(1-rho(l)) + Gz - logden(:,:,l);
% %   logpX0(:,:,l) = log(1+rho(l)*exp(Gzsmall)).*psmall + (log(1+1./(rho(l)*exp(Gzbig)))+log(rho(l))+Gzbig).*pbig - logden(:,:,l);
%             logden(:,:,:,l) = log(1+exp(Gzsmall)).*psmall + Gz.*pbig;
%             logpX1(:,:,:,l) = log(1-rho(l)) + Gz - logden(:,:,:,l);
%             logpX0(:,:,:,l) = log(1+rho(l)*exp(Gzsmall)).*psmall + (log(rho(l))+Gz).*pbig - logden(:,:,:,l);
%          else
%             % P(x_ij = 1|-) = (1-rho) * exp(Gz)/(1+exp(Gz))
%             % P(x_ij = 0|-) = rho + (1-rho) * 1/(1+exp(Gz)) = (1+rho*exp(Gz)) / (1+exp(Gz))
%             logden(:,:,:,l) = log(1+exp(Gz));
%             logpX1(:,:,:,l) = log(1-rho(l)) + Gz - logden(:,:,:,l);
%             logpX0(:,:,:,l) = log(1+rho(l)*exp(Gz)) - logden(:,:,:,l);
%          end
...
%       end
%    end
%    % conditional likelihood (Xt,yt|st,-)
%    clkl = clkl_X;
%    
%    if sum(isnan(clkl(:))) ~= 0
%       error('FFBS: NaN in clkl.');

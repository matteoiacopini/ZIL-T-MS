function [Gammas] = SimAnnMScl_edge_specific(DataX,DataZ,L,R,IterSA,state,rw,mu,sigma,k,pen1,pen0,penNorm,target_weight,cool)
%% SimAnnMScl  Initialization of gammas via Simulated Annealing, K=1 and edge-specific covariates
%
%  [GAMMAS] = SimAnnMScl_edge_specific(DATAX,DATAZ,L,R,ITETRSA,STATE,RW,MU,SIGMA,K,PEN1,PEN0,PENNORM,TARGET_WEIGHT,COOL)
...
%  of size (I,J) and the covariates (DATAZ) are of size (I,J,Q).
...
%   DataZ   IxJxQxT  observed vector of covariates
%   L       1x1      #regimes
%   R       1x1      tensor rank
      ...
      if iter==1
         ...
         for l=1:L
            ...
            for s=1:R
               ...
               normgs = normgs + norm(g1(:,s,l),1) + norm(g2(:,s,l),1) + norm(g3(:,s,l),1);
            end
            Gsa   = double( ktensor({g1(:,:,l), g2(:,:,l), g3(:,:,l)}) );
...
%                   GOld(:,:,t) = double( ttv(Gsa, DataZ(:,t), 3) );
                  GOld(:,:,t) = double( ttv(Gsa .* DataZ(:,:,:,t), ones(Q,1), 3) );
      ...
      for l=1:L
         ...
         Gnew  = double( ktensor({g1(:,:,l), g2(:,:,l), g3(:,:,l)}) );
...
%                GNew(:,:,t) = double( ttv(Gnew, DataZ(:,t), 3) );
               GNew(:,:,t) = double( ttv(Gnew .* DataZ(:,:,:,t), ones(Q,1), 3) );

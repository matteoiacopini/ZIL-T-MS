function [lkl,filtp,filts,smoothp,smooths,clkl] = ffbsX_edge_specific(Xt,Zt,rho,G,P,ip,scale_clkl)
%% ffbsX_edge_specific Forward-Filtering Backward-Sampling for ZIL-T-MS model with matrix observations and edge-specific covariates
%
...
%
% INPUTS
%  Xt   IxJxT   binary matrix of size IxJ of observations for each time t=1..T
%  Zt   IxJxQxT array of covariates: Z(i,j,:,t) is vector of size Qx1 for each time t=1..T
%  G    IxJxQxL coefficient tensor IxJxQ for each state l=1..L
...
%  smoothp  TxL smoothed probabilities
%  smooths  Tx1 smoothed states (smooth(t)= 1..L)
%  clkl     TxL conditional log-likelihood
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for t=1:T
   for l=1:L
      ...
      %          Gz = double( ttt(G(:,:,:,l), Zt(:,:,:,t), 3,3) );
      Gz = sum( double(G(:,:,:,l) .* Zt(:,:,:,t)), 3);
      ...
      if sum(pbig(:)) > 0
         ...
         expGzsmall = exp(Gzsmall);
         logden = log(1+expGzsmall).*psmall + Gz.*pbig;
         logpX0 = log(1+rho(l)*expGzsmall).*psmall + (log(rho(l))+Gz).*pbig - logden;
      else
         % P(x_ij = 1|-) = (1-rho) * exp(Gz)/(1+exp(Gz))
         % P(x_ij = 0|-) = rho + (1-rho) * 1/(1+exp(Gz)) = (1+rho*exp(Gz)) / (1+exp(Gz))
         expGz = exp(Gz);
         logden = log(1+expGz);
         logpX0 = log(1+rho(l)*expGz) - logden;
      end
